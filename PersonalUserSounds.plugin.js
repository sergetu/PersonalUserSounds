...
(function (global) {
    ...
    var PUSS_TEXT = {
        en: {
            ...
            modeHelpIntro: "Each event can use its own mode. The mode decides what happens when that user's event is detected.",
        ...
        ru: {
            ...
            modeHelpIntro: "У каждого события может быть свой режим. Режим определяет, что произойдёт, когда плагин заметит событие этого пользователя.",
    ...
    PussUI.prototype.buildEventCard = function (eventType, config, opts) {
        ...
        var modeSelect = card.querySelector('[data-field="mode"]');
        if (modeSelect) {
            modeSelect.value = config.mode || (opts.allowInherit ? "inherit" : "default");
            if (!opts.allowInherit) {
                var inheritOption = modeSelect.querySelector('option[value="inherit"]');
                if (inheritOption) inheritOption.remove();
            }
        }
    ...
    PussUI.prototype.panelClick = function (el) {
        ...
        if (action === "compat") {
            ...
            if (pathInputC) pathInputC.value = "";
    ...
    PussUI.prototype.openUserModal = function (payload) {
        ...
        var intro = body.querySelector('[data-role="intro"]');
        if (intro) intro.textContent = this.text("modeHelpIntro");
        var chips = body.querySelector('[data-role="chips"]');
        if (chips) {
            for (var m = 0; m < PUSS_MODES.length; m++) {
                var chip = document.createElement("span");
                chip.className = "puss-chip";
                chip.textContent = this.text(PUSS_MODES[m]);
